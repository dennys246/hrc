...
print(preproc_scans)
subject_contrasts = []
for ind, subject_id in enumerate(subject_ids[:2]):
    ...
    theta = np.array(glm_results.theta())
...
fig, ax = plt.subplots()
mne.viz.plot_topomap(mean_contrast, preproc_scan.info, show=True, cmap='RdBu_r', vlim=(-max_abs, max_abs))
plt.title(f"P-CAT Deconvolved Contrast")

# Save and close plot
fig.savefig(f"{plot_dir}P-CAT_deconvolved_congruent-incongruent_contrast_topomap.jpg", dpi=300, bbox_inches='tight')
plt.close(fig)